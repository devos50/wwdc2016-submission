...
- (instancetype)init
{
    ...
    if (self)
    {
        fftFrameSize = 512;
        sign = -1;
        w = [self computeTwiddleFactors];
        
        fftFrameSize2 = fftFrameSize << 1;
        
        // Pre-process Bit-Reversal
        bitm_array = (int *)malloc(sizeof(int) * fftFrameSize2);
...
- (void)transformData: (double *)theData
{
    data = theData;
    [self bitreversal];
    [self calc];
...
- (double *)computeTwiddleFactors
{
    int imax = (int) (log2(fftFrameSize));
    
    double *warray = (double *) malloc(sizeof(double) * (fftFrameSize - 1) * 4);
    int w_index = 0;
    ...
    for (int i = 0,  nstep = 2; i < imax; i++) {
        ...
        double arg = M_PI / (jmax >> 1);
        double wfr = cos(arg);
        double wfi = sign * sin(arg);
    ...
    {
        w_index = 0;
        int w_index2 = ((fftFrameSize - 1) * 4) >> 1;
...
- (void)calc
{
    ...
    int i = nstep - 2;
    if (sign == -1)
        [self calcF4FWithI:i andStep:nstep];
    else
        [self calcF4IWithI:i andStep:nstep];
    
}
    
- (void)calcF2EWithI: (int)i andStep: (int)nstep
{
    int jmax = nstep;
...
- (void)calcF4FWithI: (int)i andStep: (int)nstep
{
    ...
    while (nstep < fftFrameSize2) {
        ...
        if (nnstep == fftFrameSize2) {
            ...
            [self calcF2EWithI:i andStep:nstep];
            return;
        }
        nstep <<= 2;
        int ii = i + jmax;
    ...
    // Perform Factor-4 Decomposition with 3 * complex operators and 8 +/-
    // complex operators
- (void)calcF4IWithI: (int)i andStep: (int)nstep
{
    // Factor-4 Decomposition
    ...
    // Perform Factor-4 Decomposition with 3 * complex operators and 8 +/-
    // complex operators
- (void)calcF4FEWithI: (int)i andStep: (int)nstep
{
    // Factor-4 Decomposition
    ...
    // Perform Factor-4 Decomposition with 3 * complex operators and 8 +/-
    // complex operators
- (void)calcF4IEWithI: (int)i andStep: (int)nstep
{
    // Factor-4 Decomposition
